...
export const actions = {
    ...
    try {
      ...
      if (config.type === routeDocumentTypes.stock) {
        ...
        /*if(nameSectorResult.PROP_SEKTOR) {
          ...
          nameIndustry = nameSectorResult.INDUSTRY
        }*/
      ...
      //TODO USA Stocks
      let responseStockUsa = await api.getFinancialData(config.region, config.id);
  ...
  initIndicatorsChart({ state, dispatch, commit }: { state: IDocumentState; dispatch: any; commit: any }): boolean {
    try {
      ...
      let sizePriceY = Object.keys(priceY).length
